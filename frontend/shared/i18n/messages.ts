...
export const LOCALE_TEXTS: Record<SiteLanguage, LocaleTexts> = {
  "pt-BR": {
    ...
    finance: {
      ...
      selectOneOrMorePdfs: "Selecione um ou mais PDFs. O monitor do lote vai consultar o backend a cada 2 segundos.",
      demoReadOnly: "A demo mantém esta seção somente leitura porque a análise financeira já está carregada.",
      largeBatchesCanTakeAWhile: "Lotes grandes podem levar alguns minutos. O worker continua processando em segundo plano.",
      ...
      batchProgressSubtitle: "Uma barra de progresso compacta se atualiza automaticamente enquanto o worker processa cada PDF enviado.",
      status: "Status",
      ...
      workerKeptGoingAfterFailures: "O worker continuou mesmo após falhas, então o lote ainda pode terminar com resultados parciais.",
      ...
      salaryAnalysisPersists: "Esta seção sempre carrega os contracheques salvos do PostgreSQL, então os dados sobrevivem ao refresh.",
      demoFigures:
      ...
      loadingSavedAnalysis: "Carregando análise salva do banco de dados...",
      ...
      salaryTrendExplainer: "Uma linha única mantém a tendência salarial clara e fácil de ler.",
    },
    history: {
  ...
  en: {
    ...
    finance: {
      ...
      pdfFiles: "PDF files",
      selectOneOrMorePdfs: "Select one or more PDFs. The batch monitor will poll the backend every 2 seconds.",
      demoReadOnly: "Demo mode keeps this section read-only because the financial analysis is already loaded.",
      largeBatchesCanTakeAWhile: "Large batches can take a few minutes. The worker keeps processing in the background.",
      ...
      batchProgressSubtitle: "A compact progress bar updates automatically while the worker processes each uploaded PDF.",
      status: "Status",
      processed: "Processed",
      ...
      workerKeptGoingAfterFailures: "The worker kept going after failures, so the batch can still finish with partial results.",
      ...
      salaryAnalysisPersists: "This section always loads the saved paychecks from PostgreSQL, so the data survives refresh.",
      demoFigures:
      ...
      loadingSavedAnalysis: "Loading saved analysis from the database...",
      ...
      salaryTrendExplainer: "A single line keeps the salary trend clear and easy to scan.",
    },
    history: {
